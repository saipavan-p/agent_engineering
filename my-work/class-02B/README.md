...
- [yes] I can explain why `description` helps an LLM parent choose a child.
- [yes] I can distinguish tool capability from instruction policy.
- [yes] I can show a `state_delta` created by a tool.
- [yes] I can explain `{key?}` state templating.
- [yes] I can distinguish LLM transfer from deterministic sequence.
- [yes] I can explain why loops need both an exit condition and a hard cap.
- [yes] I can identify work that is safe to run in parallel.
- [yes] I can explain how `output_key` enables a later gather stage.
